...
getAttr     (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = name
getIsIn     (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = ii
getHasCode  (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = hc
getLhsNt    (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = nt
getCon      (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = con
getField    (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = field
getRhsNt    (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = childnt
getType     (CRule name ii hc nt con field childnt tp pattern rhs defines owrt lazy origin uses) = tp
...
cRuleLhsInh attr nt con tp = CRule attr True False nt con _LHS Nothing (Just tp) (error "cRuleLhsInh") [] Map.empty False False "" Set.empty
cRuleTerminal :: Identifier -> NontermIdent -> ConstructorIdent -> Type -> CRule
cRuleTerminal attr nt con tp = CRule attr True False nt con _LOC Nothing (Just tp) (error ("cRuleTerminal: " ++ show (attr, nt, con, tp))) [] Map.empty False False "" Set.empty
cRuleRhsSyn :: Identifier -> NontermIdent -> ConstructorIdent -> Type -> Identifier -> NontermIdent -> CRule
cRuleRhsSyn attr nt con tp field childnt = CRule attr True False nt con field (Just childnt) (Just tp) (error ("cRuleRhsSyn: " ++ show (attr, nt, con, tp, field))) [] Map.empty False False "" Set.empty

defaultRule :: Identifier -> NontermIdent -> ConstructorIdent -> Identifier -> CRule
defaultRule attr nt con field =  CRule attr (er 1) (er 2) nt con field (er 3) (er 4) (er 5) (er 6) (er 7) (er 8) False (er 9) (er 10)
